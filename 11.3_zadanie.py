...
# Load
def load_data():
    folder = r'D:\KODILLLA\wizualizacje\db'
...
app.layout = html.Div([
    html.Div([
        dcc.Tabs(id='tabs', value='tab-1', children=[
            ...
            dcc.Tab(label='CH_SELL', value='tab-3')
        ]),
...
def render_tab1(df):
    return html.Div([
        ...
        html.Div([
            dcc.DatePickerRange(
                id='sales-range',
                start_date=df['tran_date'].min().strftime('%Y-%m-%d'),
                end_date=df['tran_date'].max().strftime('%Y-%m-%d'),
...
def render_tab3(df):
    return html.Div([
        html.H1('CH_SELL', style={'text-align': 'center'}),
        html.Div([
